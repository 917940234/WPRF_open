...
def _class_balanced_bce_with_logits(
    ...
    *,
    ...
    """
    union 的 class-balanced BCE（确定性）：对每张图自适应 pos_weight，并裁剪到上界。
    """
...
def _keep_latest_checkpoints(ckpt_dir: Path, *, keep: int = 3) -> None:
    """
    仅保留最新的若干个 checkpoint（与 unet_wprf 保持一致）。

    约定：checkpoint 文件名为 epoch_010.pt 这种形式。
    """
    if int(keep) <= 0:
...
def _worker_init_fn(cfg: UNetConfig) -> Any:
...
class UNetExperiment:
    """
    纯 UNet 基线（控制变量对比 WPRF）：
    - 目标：预测 union mask（像素级语义分割）
    - 训练 loss：BCE（3A）
    - 推理：pred_union 阈值化；用于实例渲染/可视化的 CC 与 METHOD.md 本体论一致
    """
    ...
    def _parse_cfg(self, cfg: Dict[str, Any]) -> UNetConfig:
        ...
        # 旧配置（config_resolved.json 等）可能仍包含 model/wprf/struct/loss 段；
        # 新 YAML 只保留通用字段，这些段缺失时使用与主线一致的默认值即可。
        ...
        return UNetConfig(
            ...
            seg_pos_weight_max=float(loss_cfg.get("seg_pos_weight_max", DEFAULT_SEG_POS_WEIGHT_MAX)),
    ...
    def build_model(self, *, cfg: UNetConfig, device: torch.device) -> torch.nn.Module:
        ...
        _ = device  # 保持签名一致；避免未使用参数的静态检查告警。
        return smp.Unet(
    ...
    def run(self) -> None:
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "缺少依赖 `segmentation_models_pytorch`，请先安装：`python -m pip install segmentation-models-pytorch`"
            ) from e
        ...
        for epoch in range(1, cfg.num_epochs + 1):
            ...
            n_steps = 0
            pbar = tqdm(train_loader, desc=f"[UNet] 训练 Epoch {epoch}/{cfg.num_epochs}", unit="batch")
            for batch in pbar:
                ...
                logits = model(x)  # (B,1,H0,W0)
                if logits.ndim != 4 or int(logits.shape[1]) != 1:
                    raise AssertionError(f"UNet 输出必须为 (B,1,H,W)，当前 shape={tuple(logits.shape)}")
            ...
            print(f"[UNet][Epoch {epoch:03d}] loss={avg_loss:.6f}")
    ...
    def evaluate(
        ...
        *,
        ...
        pbar = tqdm(loader, desc=f"[UNet] 评估 Epoch {epoch}", unit="image", total=limit)
        n_eval = 0
        with torch.no_grad():
            for batch in pbar:
                ...
                h1, w1 = batch["omega_size"][0]

                x = batch["image_rgb"].to(device=device)
                logits = model(x)
                u_prob_t = torch.sigmoid(logits[0, 0]).to(dtype=torch.float32)  # (H0,W0)
                ...
                # 生成 CC：像素域 Φ_px → Π_s 投影到 Ω → CC（与 METHOD.md 1B + 2 一致）
